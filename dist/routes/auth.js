    ...
    app.post('/auth/registrar', async (req, reply) => {
        ...
        const resultado = await db.transaction(async (tx) => {
            ...
            const [tenant] = await tx
                ...
                .returning();
            if (!tenant)
                throw invalido('Nao foi possivel criar o workspace.');
            const [usuario] = await tx
                ...
                .returning();
            if (!usuario)
                throw invalido('Nao foi possivel criar o usuario.');
            await tx
                .insert(categorias)
                .values(CATEGORIAS_PADRAO.map((c) => ({ ...c, tenantId: tenant.id })));
            await tx.insert(politicasSla).values(Object.keys(SLA_PADRAO).map((prioridade) => ({
                tenantId: tenant.id,
                prioridade,
