...
import { convites, conviteProjetos, projetoMembros, projetos, tenants, usuarios, } from '../db/schema.js';
import { bucketAtivo, env } from '../env.js';
import { autenticar, ehUltimoAdmin, projetosVisiveis, somenteAdmin, usuarioAtual, } from '../lib/auth.js';
...
export function estadoEfetivoConvite(convite, agora = new Date()) {
...
export function usuarioPublico(u) {
    const { senhaHash: _senha, avatarFileId: _fid, avatarCaminho: _caminho, avatarUrl: _antigo, ...resto } = u;
    return {
        ...resto,
...
export async function rotasEquipe(app) {
    ...
    app.post('/convites', {
        ...
        const pendente = await db.query.convites.findFirst({
            where: and(eq(convites.tenantId, tenantId), eq(convites.email, email), eq(convites.estado, 'pendente')),
        });
        ...
        const convite = await db.transaction(async (tx) => {
            const [criado] = await tx
                ...
                .returning();
            if (!criado)
                throw invalido('Nao foi possivel criar o convite.');
    ...
    app.post('/convites/:id/reenviar', {
        ...
        if (convite.estado === 'aceito')
            throw invalido('Este convite ja foi aceito.');
        if (convite.estado === 'revogado')
            throw invalido('Este convite foi revogado.');
    ...
    app.post('/auth/eu/avatar', {
        ...
        if (!foto)
            throw invalido('Nao consegui processar esta imagem. Tente outra.');
    ...
    app.get('/usuarios/:id/avatar', {
        ...
        if (!pessoa?.avatarCaminho || !pessoa.avatarFileId)
            throw naoEncontrado('Foto');
...
async function projetosDoTenant(tenantId, ids, tx = db) {
    if (ids.length === 0)
        return [];
...
async function nomesDosProjetosDoConvite(conviteId) {
...
async function listarConvites(tenantId) {
    ...
    const vinculos = await db
        ...
        .from(conviteProjetos)
        .innerJoin(projetos, eq(projetos.id, conviteProjetos.projetoId))
        .where(inArray(conviteProjetos.conviteId, lista.map((c) => c.id)));
    return lista.map((c) => ({
        ...c,
...
async function despacharConvite(convite, nomesProjetos, convidadoPor, tenantId) {
