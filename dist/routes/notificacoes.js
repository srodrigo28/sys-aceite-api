...
export async function rotasNotificacoes(app) {
    ...
    app.get('/notificacoes', {
        schema: doc({
            ...
            descricao: 'Sempre escopada em quem esta com o token — **nao existe parametro de usuario aqui**, e ' +
        ...
        const { apenasNaoLidas } = validar(listaQuery, req.query);
        const filtro = and(eq(notificacoes.usuarioId, req.user.sub), apenasNaoLidas === '1' ? isNull(notificacoes.lidaEm) : undefined);
    ...
    app.post('/notificacoes/:id/lida', {
        schema: doc({
            ...
            descricao: 'Marcar a notificacao de outra pessoa responde **404**, nao 403: o filtro por usuario esta ' +
        ...
        const [atualizada] = await db
            ...
            .returning();
        if (!atualizada)
            throw naoEncontrado('Notificacao');
