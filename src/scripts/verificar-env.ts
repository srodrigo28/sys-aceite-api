...
const erro = (m: string) => console.log(`  \x1b[31mERRO\x1b[0m  ${m}`)
...
if (relatorio.carregados.length) {
  for (const c of relatorio.carregados) ok(`carregado  ${c}`)
} else {
  aviso('nenhum arquivo .env encontrado — valendo so as variaveis do processo')
}
for (const a of relatorio.ausentes) console.log(`        (nao existe) ${a}`)
...
for (const { nome, dica } of RECOMENDADAS) {
  const valor = process.env[nome]
  if (!valor) aviso(`${nome} nao definida — ${dica}`)
  else ok(`${nome} = ${valor}`)
}
...
if (!temBucket) {
  ...
  for (const v of BUCKET) if (!process.env[v]) console.log(`        falta ${v}`)
} else {
...
if (process.env.DATABASE_URL) {
  try {
    ...
    const [linha] = await sql<{ v: string }[]>`select version() as v`
    ...
    const existentes = await sql<{ nome: string }[]>`
    ...
    if (nomes.size === 0) {
      ...
      erro(
        `faltam ${faltando.length} de ${TABELAS_ESPERADAS.length} tabelas: ${faltando.join(', ')}`,
      )
      ...
      const sobrando = [...nomes].filter(
        (n) => !TABELAS_ESPERADAS.includes(n) && n !== '__drizzle_migrations',
      )
...
if (temBucket) {
  try {
    const inicio = Date.now()
    const r = await fetch(
      `${process.env.BUCKET_URL}/files?bucket=${process.env.BUCKET_SLUG ?? 'sysaceite'}&per_page=1`,
      {
        headers: { 'X-API-Token': process.env.BUCKET_TOKEN as string },
        signal: AbortSignal.timeout(20_000),
      },
    )
    if (r.ok) {
      const j = (await r.json()) as { pagination?: { total_items?: number } }
    ...
    erro(`bucket: ${(e as Error).message}`)
    problemas++
  }
}
