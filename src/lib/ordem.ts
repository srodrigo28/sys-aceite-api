 ...
 */
export function periodoDaAtividade(a: { previstoInicioEm?: Date | null; abertaEm?: Date | null }) {
  const base = a.previstoInicioEm ?? a.abertaEm ?? new Date()
  return { ano: base.getFullYear(), mes: base.getMonth() + 1 }
}

async function proximoCodigo(tenantId: string, ano: number): Promise<string> {
 ...
 */
export async function garantirOsDoMes(
  tenantId: string,
  projetoId: string,
  quando: { previstoInicioEm?: Date | null; abertaEm?: Date | null },
): Promise<string> {
  ...
  const existente = await db.query.ordensServico.findFirst({
    where: and(
      ...
      eq(ordensServico.ano, ano),
      eq(ordensServico.mes, mes),
    ),
    columns: { id: true },
  })
  ...
  if (!vencedora) throw new Error('nao foi possivel resolver a O.S. do mes')
  return vencedora.id
}
 ...
 */
export async function totaisDaOs(osId: string) {
  const [linha] = await db
    .select({
      atividades: sql<number>`count(*)::int`,
      minutosEstimados: sql<number>`coalesce(sum(${atividades.minutosEstimados}), 0)::int`,
      minutosApontados: sql<number>`coalesce(sum(${atividades.minutosApontados}), 0)::int`,
      concluidas: sql<number>`count(*) filter (where ${atividades.status} = 'finalizado')::int`,
      minutosConcluidos: sql<number>`coalesce(sum(${atividades.minutosApontados}) filter (where ${atividades.status} = 'finalizado'), 0)::int`,
      faturaveis: sql<number>`count(*) filter (where ${atividades.status} = 'finalizado' and ${atividades.aprovado})::int`,
      minutosFaturaveis: sql<number>`coalesce(sum(${atividades.minutosApontados}) filter (where ${atividades.status} = 'finalizado' and ${atividades.aprovado}), 0)::int`,
      emAtraso: sql<number>`count(*) filter (where ${atividades.previstoFimEm} < now() and ${atividades.status} <> 'finalizado')::int`,
      colaboradores: sql<number>`count(distinct ${atividades.id})::int`,
    })
    .from(atividades)
