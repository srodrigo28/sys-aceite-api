...
import type { Anexo } from '../db/schema.js'
import type { ConteudoArquivo } from './bucket.js'
...
type Assinatura = { bytes: number[]; deslocamento?: number; tipo: string }
...
function combina(buffer: Buffer, a: Assinatura): boolean {
  const inicio = a.deslocamento ?? 0
  if (buffer.length < inicio + a.bytes.length) return false
...
function pareceTexto(buffer: Buffer): boolean {
  const amostra = buffer.subarray(0, 4096)
  if (amostra.includes(0)) return false // byte nulo = binario
  try {
...
export function validarArquivo(
  ...
  if (buffer.byteLength === 0) throw invalido('O arquivo esta vazio.')
  ...
  if (detectado) {
    if (!(TIPOS_ACEITOS as readonly string[]).includes(detectado)) {
 ...
 */
export function anexoPublico(a: Anexo) {
  return {
    id: a.id,
    nome: a.nome,
    mimeType: a.mimeType,
 ...
 */
export function versaoParaServir(
  a: Anexo,
  querMiniatura: boolean,
): { fileId: string; storage: 'bucket' | 'disco'; caminho: string; mimeType: string; tamanho: number; checksum: string | null } {
...
export type AnexoPublico = ReturnType<typeof anexoPublico>
 ...
 */
export function arquivosDoAnexo(a: Anexo) {
  const lista = [
    {
      fileId: a.fileId,
      storage: a.storage,
 ...
 */
export function responderArquivo(
  reply: FastifyReply,
  anexo: Pick<Anexo, 'nome' | 'mimeType' | 'checksum'>,
  conteudo: ConteudoArquivo,
  opcoes: { cache: string; anexar?: boolean },
): FastifyReply {
  ...
  if (anexo.checksum) reply.header('etag', `"${anexo.checksum}"`)
