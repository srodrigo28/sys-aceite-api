...
export function podeGerarMiniatura(mimeType: string): boolean {
 ...
 */
export async function encolherParaLimite(
  original: Buffer,
  mimeType: string,
  limite: number,
): Promise<Miniatura | null> {
  if (!podeGerarMiniatura(mimeType)) return null
  if (original.byteLength <= limite) return null

  // do menos destrutivo para o mais: primeiro qualidade, depois dimensao
  const tentativas: Array<{ largura: number; qualidade: number }> = [
  ...
  for (const { largura, qualidade } of tentativas) {
    try {
      ...
      console.error(`[imagem] encolher falhou (${largura}px q${qualidade}): ${(e as Error).message}`)
      return null
    }
  }
  return null
 ...
 */
export async function prepararAvatar(original: Buffer, mimeType: string): Promise<Miniatura | null> {
  if (!podeGerarMiniatura(mimeType)) return null
  ...
  try {
    ...
    console.error(`[avatar] falhou (${mimeType}): ${(e as Error).message}`)
    return null
  }
}
...
export async function gerarMiniatura(
  ...
  try {
    ...
    if (buffer.byteLength >= original.byteLength) return null

    return { buffer, mimeType: MIME_MINIATURA, tamanho: buffer.byteLength }
  } catch (e) {
    console.error(`[miniatura] falhou (${mimeType}): ${(e as Error).message}`)
    return null
  }
}
