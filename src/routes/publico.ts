...
export async function rotasPublicas(app: FastifyInstance): Promise<void> {
  ...
  app.get('/publico/aprovacao/:token/anexos/:anexoId', {
    ...
    if (!link) throw naoEncontrado('Arquivo')

    const estado = estadoEfetivo(link)
    if (estado === 'expirado' || estado === 'revogado') throw naoEncontrado('Arquivo')
    if (!link.mostrarAnexos) throw naoEncontrado('Arquivo')
  ...
  app.post('/publico/convite/:token', {
    ...
    const criado = await db.transaction(async (tx) => {
      ...
      const [travado] = await tx
        ...
        .returning()
      if (!travado) throw conflito('Este convite ja foi usado.')
      ...
      if (ocupado) throw conflito('Ja existe uma conta com este e-mail.')
      ...
      const [usuario] = await tx
        ...
        .returning()
      if (!usuario) throw invalido('Nao foi possivel criar a conta.')
