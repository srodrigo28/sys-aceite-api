...
import { autenticar, projetosVisiveis, somenteAdmin, usuarioAtual } from '../lib/auth.js'
...
const PASSOS = ['aberta', 'em_execucao', 'em_revisao', 'aprovada', 'finalizada'] as const
...
export async function rotasOrdens(app: FastifyInstance): Promise<void> {
  ...
  app.get('/ordens', {
    schema: doc({
      ...
      descricao:
        'Uma O.S. por projeto por mes. Ela **nasce sozinha** na primeira atividade do mes — ' +
    ...
    if (visiveis !== 'todos' && visiveis.length === 0) return { ordens: [] }
  ...
  app.get('/ordens/:id', {
    schema: doc({
      ...
      descricao:
        'Alimenta a tela de fluxo de execucao: o stepper, os indicadores, o mapa de ' +
  ...
  app.patch('/ordens/:id', {
    ...
    schema: doc({
      ...
      descricao:
        'O passo so anda **para frente e de um em um**: pular de `aberta` para `aprovada` da 400. ' +
    ...
    if (dados.status && dados.status !== antes.status) {
      ...
      if (para < de) throw invalido('A O.S. nao volta de passo.')
      if (para > de + 1) throw invalido('O passo anda de um em um.')
    ...
    if (dados.responsavelId !== undefined) {
      if (dados.responsavelId) {
        const pessoa = await db.query.usuarios.findFirst({
          where: and(
            eq(usuarios.id, dados.responsavelId),
            eq(usuarios.tenantId, req.user.tenantId),
            eq(usuarios.ativo, true),
          ),
          columns: { id: true },
        })
        if (!pessoa) throw naoEncontrado('Responsavel')
      }
...
async function buscarVisivel(id: string, req: Parameters<typeof usuarioAtual>[0]) {
  ...
  if (!ordem) throw naoEncontrado('O.S.')

  const visiveis = await projetosVisiveis(req)
  if (visiveis !== 'todos' && !visiveis.includes(ordem.projetoId)) throw naoEncontrado('O.S.')
  return ordem
}
