...
export async function rotasGrupos(app: FastifyInstance): Promise<void> {
  ...
  app.post('/grupos', {
    ...
    if (repetido) throw conflito('Ja existe um grupo com este nome.')
    ...
    const [criado] = await db
      ...
      .returning()
    if (!criado) throw invalido('Nao foi possivel criar o grupo.')
  ...
  app.patch('/grupos/:id', {
    ...
    if (dados.nome) {
      ...
      if (repetido && repetido.id !== id) throw conflito('Ja existe um grupo com este nome.')
    }
    ...
    const [atualizado] = await db
      ...
      .returning()
    if (!atualizado) throw naoEncontrado('Grupo')
  ...
  app.delete('/grupos/:id', {
    ...
    if (!removido) throw naoEncontrado('Grupo')
  ...
  app.put('/grupos/:id/membros', {
    ...
    if (ids.length > 0) {
      ...
      if (doTenant.length !== ids.length) throw naoEncontrado('Usuario')
    }
...
async function buscarGrupo(id: string, tenantId: string) {
  const grupo = await db.query.grupos.findFirst({
    where: and(eq(grupos.id, id), eq(grupos.tenantId, tenantId)),
  })
  if (!grupo) throw naoEncontrado('Grupo')
  return grupo
}
