...
const MODELOS: Array<[string, string, Prioridade, Nivel, StatusOS, number, TipoOS]> = [
...
async function semear() {
  ...
  const equipe = await db
    ...
    .values([
      ...
      ...EQUIPE.map((u) => ({ ...u, tenantId: tenant.id, senhaHash, papel: 'membro' as const })),
    ])
    .returning()
  ...
  const politicas = await db
    .insert(politicasSla)
    .values(
      (Object.keys(SLA_PADRAO) as Prioridade[]).map((prioridade) => ({
        tenantId: tenant.id,
        prioridade,
